...
// Obtener uno por id
    booksRouter.get('/:id', async (request : Request, response: Response, next : NextFunction) => {
    try {
...
// Actualizacion parcial
    booksRouter.patch('/:id', async (request : Request, response : Response, next : NextFunction) => {
        try {
            const bookId = Number(request.params.id);
            const body = request.body;
...
// Actualizacion total
    booksRouter.put('/:id', async (request : Request, response : Response, next : NextFunction) => {
        try {
            const bookId = Number(request.params.id);
            const body = request.body;
...
// Eliminar uno por id
    booksRouter.delete('/:id', async (request : Request, response : Response, next :NextFunction) => {
    try {
        const bookId = Number(request.params.id);
