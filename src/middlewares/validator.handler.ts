...
export function validationHandler<T extends object>(dtoClass: { new (): T }) {
	return async (request: Request, _response: Response, next: NextFunction) => { //Retornará la función middleware asíncronica que Express ejecutará.
		try {
			...
			if (errors.length > 0) {
				const messages = errors.flatMap((error) => //> Extrae todos los mensajes de error en un solo array de strings.
